...
class UserAuthenticationController extends Controller
{
    ...
    public function token_refresh(Request $request) { 
        ...
        $expiration = new Carbon($user_authentication->refresh_token_expires_at);
        if($user_authentication->refresh_token == $request->refresh_token && $expiration->gt(Carbon::now())){
            ...
            $user_authentication->refresh_token_expires_at = $user->mail == '' ? new DateTime(strval(config('const.REFRESH_TOKEN_EXPIRATION_WITHOUT_LOGIN'))) : now()->addDays(config('const.REFRESH_TOKEN_EXPIRATION'));
